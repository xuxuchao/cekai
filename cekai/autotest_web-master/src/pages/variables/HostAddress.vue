        ...
        <el-container>
            ...
            <el-container>
                <el-main style="padding: 0; margin-left: 10px; margin-top: 10px;">
                    <div style="position: fixed; bottom: 0; right:0; left: 220px; top: 100px">
                        <el-table
        ...
        methods: {
            ...
            handleDelHost(index) {
                this.$confirm('此操作将永久删除该域名，是否继续?', '提示', {
                    ...
                    configpage.deleteHost(index).then(resp => {
                        this.getHostIPList();
                        this.currentPage = 1;
                        if (resp.success) {
            ...
            handleCurrentChange(val) {
                configpage.getHostPaginationBypage({
                    params: {
            ...
            handleConfirm(formName) {
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        ...
                        configpage.addHostIP(this.variablesForm).then(resp => {
                            if (!resp.success) {
                                this.$message.error({
                                    message: resp.msg,
                                    duration: 1000
            ...
            getpermissions() {
                getpermissions({app_label:"test_runner",model:"hostip"}).then(resp => {
            ...
            handleEditConfirm(formName) {
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        ...
                        configpage.updateHost(this.editVariablesForm.id, this.editVariablesForm).then(resp => {
                            this.getHostIPList();
                            this.currentPage = 1;
                            if (!resp.success) {
                                this.$message.error({
