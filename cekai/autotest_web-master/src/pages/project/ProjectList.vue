...
<script>
 import {getProjectList,addProject,deleteProject,updateProject,getProjectDetail,getPagination} from "../../restful/project";
        ...
        data() {
            return {
                ...
                projectForm: {
                    name: '',
                    desc: '',
                    responsible: this.$store.state.useractualname,                
                    id: ''
                },
                rules: {
                    name: [
        ...
        methods: {
            ...
            handleDelete(index, row) {
                this.$confirm('此操作将永久删除该项目, 是否继续?', '提示', {
                    ...
                    deleteProject({data: {"id": row["id"]}}).then(resp => {
                        if (resp['success']) {
                            this.success(resp);
                            this.getProjectList();
                        } else {
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        ...
                        if (this.projectForm.id === '') {
                            obj = addProject(this.projectForm);
                        } else {
                            obj = updateProject(this.projectForm);
                        }
                        obj.then(resp => {
                            if (resp.success) {
                                this.success(resp);
            ...
            getpermissions() {
                getpermissions({app_label:"test_runner",model:"project"}).then(resp => {
