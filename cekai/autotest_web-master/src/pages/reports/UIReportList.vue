        ...
        <el-container>
            ...
            <el-container>
                ...
                >
                    ...
                    <div style="position: fixed; bottom: 0px; right:0; left: 230px; top: 150px;">
                        <el-table
                        ...
                        >
                            ...
                            <el-table-column label="系统信息" width="250" align="center">
                                <template slot-scope="scope">
                                    <el-popover trigger="hover" placement="top">
                                        <p>{{ scope.row.summary.platform.python_version}}</p>
                                        <p>Platform: {{ scope.row.summary.platform.platform }}</p>
                                        <div slot="reference" class="name-wrapper">
                                            <el-tag size="medium">系统 {{ scope.row.summary.platform.platform }}</el-tag>
                                        </div>
                                    </el-popover>
                                </template>
                            </el-table-column>
                                ...
                                <template slot-scope="scope">
                                    <el-row v-show="currentRow === scope.row">
                                        ...
                                        v-if="permissions.includes('view')"
                                          size="mini"
                                            @click="getreportdetail(scope.row.id)"
                                            title="查看"
                                        >查看
                                        </el-button>
...
<script>
    ...
    import Report  from '../../pages/reports/UIReport.vue'
    ...
    export default {
        ...
        methods: {
            ...
            handleWatchReports(index) {
                ...
                window.focus();
                reportapi.watchSingleReports(index, {params:{project:this.$route.params.id}}).then(resp => {
                    newwindow.focus();
            ...
            getpermissions() {
                getpermissions({app_label:"test_runner",model:"uireport"}).then(resp => {
                    this.permissions = resp["permissions"];
                   
                })
            },
            getreportdetail(index){
                    reportapi.getUIReportsdetail(index,{
                    params: {
            ...
            handleCurrentChange(val) {
                reportapi.getUIReportsPaginationBypage({
                    params: {
            ...
            handleDelReports(index) {
                this.$confirm('此操作将永久删除该测试报告，是否继续?', '提示', {
                    ...
                    reportapi.deleteUIReports(index,{params:{project:this.$route.params.id}}).then(resp => {
            ...
            delSelectionReports() {
                if (this.selectReports.length !== 0) {
                    this.$confirm('此操作将永久删除勾选的测试报告，是否继续?', '提示', {
                        ...
                        reportapi.deleteAllUIReports(this.selectReports,{project:this.$route.params.id}).then(resp => {
