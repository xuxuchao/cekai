    ...
    <el-container>
        ...
        <el-container>
            ...
            <el-main style="padding: 0; margin-left: 10px; margin-top: 10px;">
                <div style="position: fixed; bottom: 0; right:0; left: 230px; top: 100px">
                    <el-table
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getpermissions() {
                getpermissions({app_label:"djcelery",model:"taskmeta"}).then(resp => {
            ...
            handleCopySchedule(id){
                this.$prompt('请输入定时任务名称', '提示', {
                    ...
                    taskapi.copySchedule(
                        {project: this.$route.params.id,tasktype:"UI"},
                        {
                            tasktype:"UI",
                            name: value,
                            id: id
            ...
            delTasks(id) {
                this.$confirm('此操作将永久删除该定时任务，是否继续?', '提示', {
                    ...
                    taskapi.deleteTasks(id,{params:{project:this.$route.params.id,tasktype:"UI"}}).then(resp => {
                        if (resp.status === 204){
            ...
            delselectTasks(){
                if (this.selectTasks.length !== 0){
                    this.$confirm('此操作将永久删除选中定时任务，是否继续?', '提示', {
                        ...
                        taskapi.deleteSelectTasks({project:this.$route.params.id,tasktype:"UI"},this.selectTasks).then(resp => {
                            if (resp.status === 204){
            ...
            handleRunSchedule(id){
                this.$confirm('此操作将运行此定时任务并发送邮件, 是否继续?', '提示', {
                    ...
                    this.loading = true;
                    taskapi.runScheduleTest(id,{params:{tasktype: "UI"}}).then(resp => {
            ...
            getTaskList() {
                taskapi.taskList({params: {project: this.$route.params.id,tasktype: "UI"}}).then(resp => {
                    this.tasksData = resp;
                    this.loading = false
                })
            },
