                ...
                <div>
                    ...
                    >
                        ...
                        >
                            <template slot-scope="scope"
                            >
                                <div v-if="scope.row.summary_kwargs.iswecatsend==true">是</div>
                                <div v-if="scope.row.summary_kwargs.iswecatsend==false">否</div>
                            </template>
                        </el-table-column>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getpermissions() {
                getpermissions({app_label:"djcelery",model:"periodictask"}).then(resp => {
            ...
            handleCopySchedule(id){
                this.$prompt('请输入定时任务名称', '提示', {
                    ...
                    taskapi.copySchedule(
                        {project: this.$route.params.id,tasktype: 'API'},
                        {tasktype: 'API',
                            name: value,
                            id: id
                        }
            ...
            delTasks(id) {
                this.$confirm('此操作将永久删除该定时任务，是否继续?', '提示', {
                    ...
                    taskapi.deleteTasks(id,{params:{project:this.$route.params.id,tasktype:"API"}}).then(resp => {
                        if (resp.status === 204){
            ...
            delselectTasks(){
                if (this.selectTasks.length !== 0){
                    this.$confirm('此操作将永久删除选中定时任务，是否继续?', '提示', {
                        ...
                        taskapi.deleteSelectTasks({project:this.$route.params.id,tasktype:"API"},this.selectTasks).then(resp => {
                            if (resp.status === 204){
            ...
            handleRunSchedule(id){
                this.$confirm('此操作将运行此定时任务并发送邮件, 是否继续?', '提示', {
                    ...
                    this.loading = true;
                    taskapi.runScheduleTest(id,{params:{tasktype: "API"}}).then(resp => {
            ...
            handleCurrentChange(val) {
                taskapi.getTaskPaginationBypage({
                    params: {
            ...
            handleEditSchedule(id, index_data){
                ...
                this.ruleForm["iswecatsend"]=index_data.summary_kwargs.iswecatsend;
                this.ruleForm["webhook"]=index_data.summary_kwargs.webhook;
                ...
                this.testData=index_data.summary_args;
            ...
            getTaskList() {
                taskapi.taskList({params: {project: this.$route.params.id,tasktype: "API"}}).then(resp => {
